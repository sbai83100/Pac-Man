...
title = pygame.mixer.Sound("pacman_title.wav")
title_channel = pygame.mixer.Channel(0)
title_channel.play(title)
beginning = pygame.mixer.Sound("pacman_beginning.wav")
beginning_channel = pygame.mixer.Channel(1)
pellet_sound = pygame.mixer.Sound("pacman_chomp.wav")
pellet_channel = pygame.mixer.Channel(2)
death = pygame.mixer.Sound("pacman_death.wav")
death_channel = pygame.mixer.Channel(3)
...
""" SPRITE CLASSES """
# PacMan, the main character; deals with movement and wall, pellet, and ghost collisions
class PacMan(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # Move Pac-Man left/right
        self.rect.x += self.move_x

        # If Pac-Man hits the edges, he wraps around the screen.
        ...
        # Check and see if we hit a pellet
...
# Ghost, the enemy; deals with movement and wall and friend collisions
class Ghost(pygame.sprite.Sprite):
    def __init__(self, x, y, color, name):
        ...
        # Set the width and height of ghost
    ...
    def update(self):
        ...
        # Move ghost left/right
        self.rect.x += self.move_x

        # If ghost hits the edges, he wraps around the screen.
        if self.rect.x == 0 and self.rect.y == 225 and self.move_x < 0:
            self.rect.x = 500
        if self.rect.x == 500 and self.rect.y == 225 and self.move_x > 0:
            self.rect.x = 0

        # Check and see if ghost hit a wall
        block_hit_list = pygame.sprite.spritecollide(self, self.walls, False)
        for block in block_hit_list:
            # If we are moving right, set our right side to the left side of the wall we hit
            if self.move_x >= 0:
...
# Wall, limits PacMan and Ghost movement
...
# Pellet, what PacMan is trying to eat all of
...
# Buttons facilitate user interaction and game flow
class Button:
...
# Resets all graphics and sprites
def reset():
    global lives
    ...
    # creating ghost and friends list for PacMan and ghosts
